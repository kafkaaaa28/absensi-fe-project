  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="relative rounded-xl overflow-hidden h-52 md:h-64 shadow-lg mb-5">
        <img src={bgAsdos} alt="dashboard background" className="absolute inset-0 w-full h-full object-cover object-[center_65%] z-0" />
        ...
        <div className="relative z-20 h-full w-full p-6 md:p-8 flex flex-col md:flex-row justify-between items-center text-white">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold">
              Selamat Datang, <span className="text-blue-300"> {loading ? <Spinner color="success" size="md" aria-label="Loading" /> : isAsdos?.nama}</span> 👋
            </h2>
            <p className="text-white/80 mt-1 text-sm">Dashboard terbaru Anda ada di sini</p>
          </div>

          <div className="mt-4 md:mt-0">
            <span className="inline-block bg-gradient-to-r from-green-400 to-green-600 text-white text-sm font-semibold py-2 px-4 rounded-lg shadow-md">Asdos Panel</span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow p-6 hover:shadow-xl transition duration-300">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-700">{loading ? <Spinner color="info" aria-label="Info spinner example" /> : 0}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-6 hover:shadow-xl transition duration-300">
