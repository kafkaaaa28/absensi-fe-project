...
function TambahJadwal({ isOpen, dataKelas, setIsOpen, onSubmit, loading }) {
  ...
  const handleChange = (e) => {
    ...
    if (e.target.name === 'id_kelas') {
      const selected = dataKelas.find((item) => item.id_kelas.toString() === e.target.value);
      if (selected) {
        setFormData({
          ...formData,
      ...
      <Modal show={isOpen} onClose={handleClose} size="md">
        ...
        <ModalBody>
          <div className="space-y-4">
            ...
            <div>
              <TextInput id="jam_mulai" name="jam_mulai" type="text" value={formData.sks === undefined ? `Pilih Kelas terlebih Dahulu` : `${formData.sks} sks`} disabled />
            </div>
            <div>
