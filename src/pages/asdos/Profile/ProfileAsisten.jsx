...
import { FaEnvelope, FaGraduationCap, FaIdCard, FaUniversity, FaUserEdit, FaLock, FaBookOpen, FaCalendarAlt, FaCog, FaClipboardList, FaCertificate, FaBell, FaDoorOpen } from 'react-icons/fa';
import { infoAlert } from '../../../utils/alerts';
import LoadingPage from '../../../components/common/LoadingPage';

const ProfileAsisten = () => {
  ...
  const quickActions = [
    ...
    {
      icon: <FaDoorOpen className="text-green-500 text-2xl" />,
      title: 'Kelas',
      action: () => navigate('/dashboardAsisten/kelas'),
    },
    {
      icon: <FaCalendarAlt className="text-purple-500 text-2xl" />,
      title: 'Jadwal',
      action: () => navigate('/dashboardAsisten/jadwal'),
    },
  ];
  const menuItems = [
    {
    ...
    },
    {
      ...
      title: 'Nim',
      value: !user.nim ? 'bukan mahasiswa' : user.nim,
      color: 'bg-blue-50',
    },
    {
      icon: <FaUniversity className="text-green-500" />,
      title: 'Fakultas',
      value: !user.fakultas ? 'bukan mahasiswa' : user.fakultas,
      color: 'bg-green-50',
    },
    {
      icon: <FaGraduationCap className="text-purple-500" />,
      title: 'Program Studi',
      value: !user.prodi ? 'bukan mahasiswa' : user.prodi,
      color: 'bg-purple-50',
    },
    {
      ...
      value: user.create
        ? new Date(user.create).toLocaleString('id-ID', {
            day: '2-digit',
            month: 'long',
            year: 'numeric',
          })
