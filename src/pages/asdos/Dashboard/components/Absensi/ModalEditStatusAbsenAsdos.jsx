...
const ModalEditStatusAbsenAsdos = ({ data, onUpdate, modalEditstatus, OnClose, loadingUpdate }) => {
  const [formData, setFormData] = useState({ ...data });
  const { loadingSiswa, fetchAbsenSiswaHariini, dataSiswaAbsenHariini, setDataSiswaAbsenHariini } = useAbsensiAsdos();
  ...
  useEffect(() => {
    if (modalEditstatus && data?.id_jadwal && data?.id_kelas) {
      fetchAbsenSiswaHariini(data.id_kelas, data.id_jadwal);
    }
  }, [modalEditstatus]);

  const handleStatusChange = (index, newStatus) => {
    const updatedData = [...dataSiswaAbsenHariini];
    updatedData[index] = { ...updatedData[index], status: newStatus };
    setDataSiswaAbsenHariini(updatedData);
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onUpdate(dataSiswaAbsenHariini);
  };
      ...
      <ModalBody className="px-6 py-4">
        <form onSubmit={handleSubmit}>
          <div className="overflow-x-auto">
            {dataSiswaAbsenHariini.length === 0 ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {dataSiswaAbsenHariini.map((item, index) => (
...
export default ModalEditStatusAbsenAsdos;
