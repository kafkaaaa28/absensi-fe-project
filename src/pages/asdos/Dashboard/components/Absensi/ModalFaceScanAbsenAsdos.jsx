...
import { ApiUpdateAbsenFaceAsdos, ApiFaceEmbeddingAsdos } from '../../../../../api/Asdos/AbsensiAsdosApi';
const ModalFaceScanAbsenAsdos = ({ data, modalLihat, OnClose }) => {
  ...
  const loadEmbeddings = async () => {
    try {
      const res = await ApiFaceEmbeddingAsdos(data.id_kelas);
      ...
      for (let i = 0; i < faceDetections.length; i++) {
        ...
        if (match) {
          ...
          if (!processedNPMs.current.has(match.nim)) {
            ...
            try {
              // maka simpan ke db
              await ApiUpdateAbsenFaceAsdos(data.id_kelas, data.id_jadwal, {
...
export default ModalFaceScanAbsenAsdos;
