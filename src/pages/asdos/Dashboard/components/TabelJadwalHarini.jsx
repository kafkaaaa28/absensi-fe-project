            ...
            data.map((item, index) => (
              <tr key={item.id_jadwal} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="flex space-x-2">
                    <button
                      className="bg-purple-100 flex items-center gap-1 sm:gap-1.5
             text-purple-500
             text-[10px] sm:text-xs md:text-sm
             px-1.5 sm:px-2 md:px-2.5
             py-0.5 sm:py-1
             rounded hover:bg-purple-200"
                      onClick={() => onOpenScanface(item)}
                    >
                      <TbUserScan className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5" />
                      <span className="hidden sm:inline">Face Scan</span>
                    </button>
                    <button
                      className="bg-blue-100 flex items-center gap-1 sm:gap-1.5
             text-blue-500
             text-[10px] sm:text-xs md:text-sm
             px-1.5 sm:px-2 md:px-2.5
             py-0.5 sm:py-1
             rounded hover:bg-blue-200"
                    ...
                    >
                      <MdOutlineQrCode2 className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5" />
                      <span className="hidden sm:inline">QR Code</span>
                    </button>

                    <button
                      className="bg-yellow-100 flex items-center gap-1 sm:gap-1.5
             text-yellow-500
             text-[10px] sm:text-xs md:text-sm
             px-1.5 sm:px-2 md:px-2.5
             py-0.5 sm:py-1
             rounded hover:bg-yellow-200"
                      onClick={() => onEditAbsen(item)}
                    >
                      <CiEdit className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5" />
